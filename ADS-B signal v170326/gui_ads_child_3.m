classdef gui_ads_child_3 < handle
%% *********************************************************************
%
% @file         gui_ads_child_3.m
% @brief        Class definition for button 3 GUI.
%
% ******************************************************************************
    properties
...
%         plane_edt_tm;
    ...
    methods
        ...
        function obj = gui_ads_child_3(gui_m)
            ...
            obj.gui_p = figure('Color', [0.83, 0.82, 0.78], ...
                'Numbertitle', 'off', 'Name', 'ADS-B接收机灵敏度测试程序', ...
            ...
            obj.plane_edt_vh = uicontrol('parent', obj.panel_plane, 'style', ...
                'edit', 'BackgroundColor','white' ...
              ,'Fontsize',11,'position',[361 obj.panel_height - 250 - 40 ...
              120 40]);
...
%               120 40]);
            ...
            obj.btn_c1 = uicontrol('parent', obj.gui_p, ...
                ...
                'Fontsize', 15, 'position', [floor((obj.panel_width - 400) / 2), ...
                20, 150, 50]);
            % Create handle for button "Stop programme"。
            obj.btn_c2 = uicontrol('parent', obj.gui_p, ...
                'style', 'pushbutton', 'BackgroundColor', ...
                [0.83, 0.82, 0.78], 'string', '退出', ...
                'Fontsize', 15, 'position', [floor((obj.panel_width - 450) / 2) + 280, ...
                20, 150, 50]);
...
%             set(obj.plane_edt_tm, 'string', tm);
        ...
        function button_receiver_callback(obj, source, eventdata)
            ...
            if isempty(obj.city_obj.city_name)
                set(obj.edt_echo, 'string', '请先输入城市名拼音，再获取该城市经纬度信息！');
                return;
            end
        ...
        function button_start_callback(obj, source, eventdata)
            ...
            if isempty(get(obj.rcv_pw_edt, 'string'))
                set(obj.edt_echo, 'string', '接收信号功率尚未设置，请先设置接收信号功率！');
                return;
            end
            
            pw_edt = str2double(get(obj.rcv_pw_edt, 'string'));
            if isnan(pw_edt)
                set(obj.edt_echo, 'string', '设置的接收功率中包含非法字符，应为数值，请重新设置！');
                return
            end
            ...
            set(obj.edt_echo, 'string', '正在运行“ADS-B接收机灵敏度测试程序”...');
            pause(0.3);
            ...
            ind0 = find(adsbMsgs.type == 0, 1);
            ind1 = find(adsbMsgs.type == 1, 1);
            ind2 = find(adsbMsgs.type == 2, 1);
            ind3 = find(adsbMsgs.type == 3, 1);
            tType = [0; 1; 2; 3];
            tSeries = [adsbMsgs.txTimeSeries(ind0); ...
                adsbMsgs.txTimeSeries(ind1); ...
                adsbMsgs.txTimeSeries(ind2); ...
                adsbMsgs.txTimeSeries(ind3)];
            Trace = [adsbMsgs.traceSeries(ind0, :); ...
                adsbMsgs.traceSeries(ind1, :); ...
                adsbMsgs.traceSeries(ind2, :); ...
                adsbMsgs.traceSeries(ind3, :)];
            Bitmessage = [adsbMsgs.bitStream(ind0, :); ...
                adsbMsgs.bitStream(ind1, :); ...
                adsbMsgs.bitStream(ind2, :); ...
                adsbMsgs.bitStream(ind3, :)];
            [tSort, tInd] = sort(tSeries);
            adsbMsgs.txTimeSeries = tSort;
            adsbMsgs.type = tType(tInd);
            adsbMsgs.traceSeries = Trace(tInd, :);
            adsbMsgs.bitStream = Bitmessage(tInd, :);
            
            rcv_power = str2double(get(obj.rcv_pw_edt, 'string'));
            receiver.receiveMessages(adsbMsgs);
            adsbMsgs.rxPower = repmat(rcv_power, 4, 1);
