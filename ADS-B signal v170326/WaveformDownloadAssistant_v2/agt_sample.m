...
instid='192.168.1.16'; % use this to set the IP address of the source to use
...
f1 = 1e6;
f2 = 3e6;
IQData = [ones(1,0.5*32) zeros(1,1*0.5*32)];
IQData = [IQData IQData IQData];
% IQData = zeros(1,1500-98);
% IQData = [exp( j * 2 * pi * f1 * t ) zeros(1,1000)];% + ...
		 %exp( j * 2 * pi * f2 * t );
...
[status, status_description] = agt_waveformload(io, IQData, 'agtsample', fs, 'play', 'no_normscale', Markers);
